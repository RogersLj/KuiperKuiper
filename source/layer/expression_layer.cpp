...
void ExpressionLayer::Forward(const std::vector<std::shared_ptr<Tensor<float>>> &inputs, std::vector<std::shared_ptr<Tensor<float>>> &outputs) {
    ...
    CHECK(this->parser_ != nullptr) << "parser is null";

    // 栈存储的是一个一个batch的输入
    std::stack<std::vector<std::shared_ptr<Tensor<float>>>> oprand_stack;
    // 遍历token列表，遇到数字就入栈，遇到操作符就将两个栈顶元素出栈，并计算
    const std::vector<std::shared_ptr<TokenNode>>& nodes = this->parser_->Generate();
...
void ExpressionLayer::CreateInstance(const std::shared_ptr<RuntimeOperator> &op, std::shared_ptr<Layer>& expression_layer) {
    const auto& params = op->params;

    const auto& expr = dynamic_cast<RuntimeParameterString* >(params.at("expr"))->value;

    expression_layer = std::make_shared<ExpressionLayer>(expr);
}

LayerRegisterWrapper kExpressionLayer("pnnx.Expression", ExpressionLayer::CreateInstance);
