...
class Model(nn.Module):
    def __init__(self):
        ...
        self.relu = nn.ReLU()
        self.maxpool = nn.MaxPool2d(ceil_mode=False, dilation=(1,1), kernel_size=(2,2), padding=(0,0), return_indices=False, stride=(2,2))

        archive = zipfile.ZipFile('expr_layer.pnnx.bin', 'r')
...
def export_torchscript():
    ...
    mod.save("expr_layer_pnnx.py.pt")
...
def export_onnx():
    ...
    torch.onnx._export(net, v_0, "expr_layer_pnnx.py.onnx", export_params=True, operator_export_type=torch.onnx.OperatorExportTypes.ONNX_ATEN_FALLBACK, opset_version=13, input_names=['in0'], output_names=['out0'])
